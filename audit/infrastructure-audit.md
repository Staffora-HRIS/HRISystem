...
**Monorepo:** Bun workspaces with packages `@staffora/api`, `@staffora/web`, `@staffora/shared`, and `@staffora/website`.
...
- **Issue [Low]:** Upstream `website_backend` references `server website:5174` but there is no `website` service in docker-compose
...
- **Workspaces:** `packages/*` and `Website/`
...
| `@staffora/website` | `react-router build` (Vite) | N/A | Marketing site |
