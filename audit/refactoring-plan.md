...
3. **`@better-auth/infra`** in `Website/package.json` -- Zero imports found across the Website codebase.

4. **Duplicate `ServiceResult` type** in 7 test files -- Each re-declares the interface instead of importing from `packages/api/src/types/service-result.ts`.

5. **Duplicate cookie helper functions** (`buildCookieHeader`, `splitCombinedSetCookieHeader`) across multiple route test files.

### Files Affected

- `packages/web/src/App.tsx` (delete)
- `migrations/fix_schema_migrations_filenames.sql` (archive / add comment)
- `Website/package.json` (remove `@better-auth/infra`)
...
**3. Remove unused Website dependency:**

In `Website/package.json`, remove:
```json
"@better-auth/infra": "^0.1.8",
```

Then run `bun install` in the Website directory.
