...
All diagrams in this file use Mermaid `stateDiagram-v2` syntax. The following conventions apply:

| Element | Visual | Meaning |
|---------|--------|---------|
| `[*]` (filled circle) | Solid black dot | Initial state (entry point) when an arrow leads away, or terminal state (end) when an arrow leads in |
| Named rectangle | Rounded box | A discrete state in the entity lifecycle (e.g., `pending`, `active`, `terminated`) |
| Arrow with label | `-->` with text | State transition; the label describes the triggering action (e.g., "Activate", "Escalate") |
| Terminal state | Arrow pointing to `[*]` | The entity cannot transition further from this state (e.g., `terminated`, `closed`, `cancelled`) |

### Reading the Diagrams

- Follow arrows from the initial `[*]` to trace valid lifecycle paths.
- Each arrow label is the action that causes the transition.
- States with no outgoing arrows (other than to `[*]`) are terminal -- once reached, the entity's lifecycle is complete.
- The tables below each diagram list transition requirements (e.g., whether a reason, effective date, or approval is needed).
