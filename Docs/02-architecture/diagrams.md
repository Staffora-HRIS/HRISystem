...
This file uses several Mermaid diagram types. The legend below explains the visual conventions used throughout.

### Flowchart Diagrams (graph TB / graph TD / graph LR)
...
| Dashed arrow | `-.->` | Asynchronous or event-driven connection |
| Arrow label | `\|"text"\|` | Description of the interaction or protocol |

### Node Color Coding (style)

Individual nodes are colored to indicate their role. Colors vary by diagram; the most common assignments are:

| Color | Hex | Role | Examples |
|-------|-----|------|----------|
| Blue | `#4a9eff` | Infrastructure or API | CORS, API Server, hris_app role, outbox "Created" state |
| Green | `#51cf66` | Database / security policies | DB Plugin, RLS SELECT/INSERT policies, Nginx, outbox "Processed" state |
| Yellow | `#ffd43b` | Cache / bypass mechanisms | Cache Plugin, System Context Bypass, QueryClient |
| Purple | `#cc5de8` | Auth or frontend | Auth Plugin, Web Frontend, BaseWorker |
| Orange | `#ff922b` | Tenant or worker | Tenant Plugin, Outbox Poller, Background Worker |
| Teal | `#20c997` | Authorization | RBAC Plugin |
| Red | `#ff6b6b` | Errors or failures | Errors Plugin, outbox "Failed" state |
| Dark blue | `#336791` | PostgreSQL | PostgreSQL containers |
| Dark red | `#dc382d` | Redis | Redis containers |
| Gray | `#868e96` | Dead letter / terminal | Dead Letter Queue |
...
| `alt` / `else` block | Conditional branching (e.g., cache hit vs. miss, valid vs. invalid) |
| `loop` block | Repeated operation (e.g., polling interval) |

### State Machine Diagrams (stateDiagram-v2)

| Element | Meaning |
|---------|---------|
| `[*]` (filled circle) | Initial state (start) or terminal state (end) |
| Named box | A discrete state in the lifecycle |
| Arrow with label | State transition with the triggering action |
| Nested `state` block | Sub-states within a parent state |
...
| `}o--\|\|` | Many-to-one relationship (reverse notation) |
| `\|\|--o{` ... `\|\|--o{` | Chain of one-to-many from a shared parent |
| Entity block | Database table with typed column definitions |

### Pie Chart (pie)

| Element | Meaning |
|---------|---------|
| Slice with label | Category and its count or proportion |
| Title | What the chart represents |
