...
function splitCombinedSetCookieHeader(value: string): string[] {
  // Some runtimes combine multiple Set-Cookie headers into a single string.
  // We split on ", " only when it looks like a cookie boundary (NAME=...),
  // not when it is part of Expires=Wed, 21 Oct...
  const out: string[] = [];
  let start = 0;
