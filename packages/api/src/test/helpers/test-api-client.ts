/**
 * Test API Client Utility
 *
 * High-level HTTP client for integration tests that automates the ceremony
 * of authenticated, tenant-scoped, idempotent, CSRF-protected requests.
 *
 * Usage:
 * ```ts
 * const client = await createTestApiClient(app, {
 *   db,
 *   tenantId: tenant.id,
 *   userId: user.id,
 ...
 *
 * const createRes = await client.post("/api/v1/hr/employees", { ... });
 ...
 *
 * Features:
 * - Automatic Better Auth session creation (sign-in via app.handle)
 * - Cookie persistence across requests (session token)
 * - Automatic X-Tenant-ID header injection
 * - Automatic Idempotency-Key generation for mutations (POST/PUT/PATCH/DELETE)
 * - Automatic CSRF token generation for mutations
 * - Convenience methods: get, post, put, patch, delete
 * - Cookie jar that accumulates Set-Cookie headers from responses
 * - Support for custom headers per-request
 * - Optional: skip CSRF, skip idempotency, custom idempotency key
 */
...
/** The Elysia app interface (only the handle method is needed). */
...
export interface CreateTestApiClientOptions {
   ...
   * Defaults to true. Set to false if your routes do not use requireCsrf().
   */
  includeCsrf?: boolean;
   ...
   * Useful if you want to test unauthenticated behavior first, then call login() manually.
...
class CookieJar {
  ...
  absorb(response: Response): void {
    ...
    for (const cookie of setCookies) {
      // Extract name=value (strip attributes like Path, HttpOnly, etc.)
      ...
      // Check for deletion (max-age=0 or expires in the past)
 ...
 *
 * Wraps an Elysia app instance and handles all the ceremony of session cookies,
 * tenant headers, idempotency keys, and CSRF tokens automatically.
 */
export class AuthenticatedTestClient {
  ...
  constructor(
    ...
    this.includeCsrf = options.includeCsrf;
    this.includeIdempotencyKey = options.includeIdempotencyKey;
  ...
  /** Absorb cookies from a response into the jar. */
   ...
   */
  isLoggedIn(): boolean {
    return this.cookieJar.has("staffora.session_token");
  }
  ...
  /** Send a GET request. */
  async get(path: string, options?: TestRequestOptions): Promise<Response> {
  ...
  async delete(path: string, options?: TestRequestOptions): Promise<Response> {
  ...
  async login(email: string, password: string): Promise<Response> {
    ...
    if (response.status !== 200) {
      ...
      throw new Error(
        `Test API client sign-in failed (status ${response.status}): ${text}`
      );
    }
  ...
  /**
   * Clean up resources.
   * Currently just clears cookies. Safe to call multiple times.
   */
 ...
 * 3. Returns an `AuthenticatedTestClient` ready for use
 *
 * Prerequisites:
 * - The user must already exist in `app.users` (created via `createTestUser()`)
 * - The user must have a `user_tenants` + `role_assignments` entry
...
export async function createTestApiClient(
  app: TestApp,
  options: CreateTestApiClientOptions
  ...
  const client = new AuthenticatedTestClient(app, {
    baseUrl,
    tenantId,
    userId,
    includeCsrf,
...
// =============================================================================
// Unauthenticated client factory
// =============================================================================

/**
 * Create an unauthenticated test API client.
 *
 ...
 * @example
 * ```ts
 * const anonClient = createUnauthenticatedTestClient(app, {
 *   tenantId: tenant.id,
 * });
 *
 ...
 * ```
 */
export function createUnauthenticatedTestClient(
  app: TestApp,
  options: {
    tenantId?: string;
    baseUrl?: string;
  ...
  return new AuthenticatedTestClient(app, {
    baseUrl,
    tenantId,
...
/**
 * Parse a JSON response body. Returns the parsed body or throws on failure.
...
/**
 * Assert that a response has a specific status and return the parsed JSON body.
