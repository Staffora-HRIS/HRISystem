/**
 * Core HR Module - Employee Repository
 *
 * Provides data access methods for Employee entities including
 * effective-dated records, history, overlap checking, and utilities.
...
export class EmployeeRepository {
  ...
  /**
   * Get all active position assignments for an employee (including FTE)
   */
  async findEmployeePositions(
    context: TenantContext,
    employeeId: string
  ): Promise<PositionAssignmentRow[]> {
    const result = await this.db.withTransaction(context, async (tx) => {
      const rows = await tx<PositionAssignmentRow[]>`
        SELECT pa.id, pa.tenant_id, pa.employee_id, pa.effective_from, pa.effective_to,
               pa.position_id, p.code as position_code, p.title as position_title,
               pa.org_unit_id, ou.name as org_unit_name, p.job_grade,
               pa.is_primary, pa.fte_percentage::text, pa.assignment_reason,
               pa.created_at, pa.created_by
        FROM app.position_assignments pa
        JOIN app.positions p ON pa.position_id = p.id
        JOIN app.org_units ou ON pa.org_unit_id = ou.id
        WHERE pa.employee_id = ${employeeId}::uuid
  ...
  /**
   * Find a specific position assignment by ID
   */
  async findPositionAssignmentById(
    context: TenantContext,
    assignmentId: string
  ): Promise<PositionAssignmentRow | null> {
    const result = await this.db.withTransaction(context, async (tx) => {
      const rows = await tx<PositionAssignmentRow[]>`
        SELECT pa.id, pa.tenant_id, pa.employee_id, pa.effective_from, pa.effective_to,
               pa.position_id, p.code as position_code, p.title as position_title,
  ...
  /**
   * Get total FTE percentage for an employee across all active assignments.
   * Optionally excludes a specific assignment (useful for update validation).
   */
  async getEmployeeTotalFte(
    context: TenantContext,
    employeeId: string,
    excludeAssignmentId?: string
  ): Promise<number> {
    const result = await this.db.withTransaction(context, async (tx) => {
      const rows = await tx<{ totalFte: string }[]>`
        SELECT app.get_employee_total_fte(
          ${employeeId}::uuid,
          ${excludeAssignmentId || null}::uuid
        )::text as total_fte
      `;
      return rows;
    });

    return parseFloat(result[0]?.totalFte || "0");
  }

  /**
   * Assign an additional position to an employee (concurrent employment).
   * If the new assignment is primary, existing primary positions are closed.
   */
  async assignAdditionalPosition(
    ...
    // If this is a primary assignment, close existing primary
    if (data.is_primary) {
      await tx`
        UPDATE app.position_assignments
        SET effective_to = ${data.effective_from}::date, updated_at = now()
    ...
    const rows = await tx<PositionAssignmentRow[]>`
      INSERT INTO app.position_assignments (
        tenant_id, employee_id, effective_from,
        position_id, org_unit_id, is_primary, fte_percentage,
        assignment_reason, created_by
      )
      VALUES (
        ${context.tenantId}::uuid, ${employeeId}::uuid, ${data.effective_from}::date,
        ${data.position_id}::uuid, ${data.org_unit_id}::uuid,
        ${data.is_primary}, ${data.fte_percentage},
        ${data.assignment_reason || null}, ${createdBy}::uuid
      )
      RETURNING
        id, tenant_id, employee_id, effective_from, effective_to,
        position_id, org_unit_id, is_primary, fte_percentage::text,
  ...
  /**
   * End a specific position assignment
   */
  async endPositionAssignment(
    ...
    const result = await tx`
      UPDATE app.position_assignments
      SET effective_to = ${effectiveTo}::date, updated_at = now()
      WHERE id = ${assignmentId}::uuid
        AND effective_to IS NULL
        AND effective_from < ${effectiveTo}::date
    `;
  ...
  /**
   * Get max FTE percentage from tenant settings.
   * Defaults to 100 if not configured.
   */
  async getTenantMaxFte(
    context: TenantContext
  ): Promise<number> {
    const result = await this.db.withTransaction(context, async (tx) => {
      const rows = await tx<{ maxFte: string | null }[]>`
        SELECT (settings->>'max_fte_percentage')::text as max_fte
        FROM app.tenants
    ...
    const maxFte = result[0]?.maxFte;
    return maxFte ? parseFloat(maxFte) : 100;
  }

  // ===========================================================================
  // Employment Records (Rehire History)
  // ===========================================================================

  /**
   * Get all employment records for an employee, ordered by employment number descending
   */
  async getEmploymentRecords(
    context: TenantContext,
    employeeId: string
  ): Promise<EmploymentRecordRow[]> {
    const result = await this.db.withTransaction(context, async (tx) => {
      return tx<EmploymentRecordRow[]>`
        SELECT id, tenant_id, employee_id, employment_number,
               start_date, end_date, termination_reason,
               is_current, previous_employment_id,
               created_at, updated_at
        FROM app.employment_records
        WHERE employee_id = ${employeeId}::uuid
        ORDER BY employment_number DESC
      `;
    });
    return result;
  }
  ...
  /**
   * Get the current employment record for an employee (is_current = true)
   */
  async getCurrentEmploymentRecord(
    context: TenantContext,
    employeeId: string
  ): Promise<EmploymentRecordRow | null> {
    const result = await this.db.withTransaction(context, async (tx) => {
      return tx<EmploymentRecordRow[]>`
        SELECT id, tenant_id, employee_id, employment_number,
               start_date, end_date, termination_reason,
  ...
  /**
   * Get the highest employment number for an employee within a transaction
   */
  async getMaxEmploymentNumber(
    tx: TransactionSql,
    employeeId: string
  ): Promise<number> {
    const rows = await tx<{ maxNum: number | null }[]>`
      SELECT MAX(employment_number) as max_num
      FROM app.employment_records
      WHERE employee_id = ${employeeId}::uuid
    `;
  ...
  /**
   * Close the current employment record (set end_date and is_current = false)
   */
  async closeCurrentEmploymentRecord(
    tx: TransactionSql,
  ...
  /**
   * Create a new employment record for a rehire
   */
  async createEmploymentRecord(
    ...
    const rows = await tx<EmploymentRecordRow[]>`
      INSERT INTO app.employment_records (
        tenant_id, employee_id, employment_number,
        start_date, is_current, previous_employment_id
      )
      VALUES (
        ${context.tenantId}::uuid, ${data.employeeId}::uuid, ${data.employmentNumber},
        ${data.startDate}::date, true, ${data.previousEmploymentId}::uuid
      )
      RETURNING id, tenant_id, employee_id, employment_number,
                start_date, end_date, termination_reason,
                is_current, previous_employment_id,
                created_at, updated_at
    `;
  ...
  /**
   * Ensure initial employment record exists for a newly hired employee.
   * Idempotent: returns existing record if one exists.
   */
  async ensureInitialEmploymentRecord(
    tx: TransactionSql,
    context: TenantContext,
    employeeId: string,
    startDate: string
  ): Promise<EmploymentRecordRow> {
    const existing = await tx<EmploymentRecordRow[]>`
      SELECT id, tenant_id, employee_id, employment_number,
             start_date, end_date, termination_reason,
             is_current, previous_employment_id,
             created_at, updated_at
    ...
    const rows = await tx<EmploymentRecordRow[]>`
      ...
      VALUES (
        ${context.tenantId}::uuid, ${employeeId}::uuid, 1,
        ${startDate}::date, true
      )
      RETURNING id, tenant_id, employee_id, employment_number,
                start_date, end_date, termination_reason,
  ...
  /**
   * Rehire an employee: set status to pending, update hire_date, clear termination fields
   */
  async rehireEmployee(
    ...
    const result = await tx`
      UPDATE app.employees
      SET status = 'pending'::app.employee_status,
          hire_date = ${rehireDate}::date,
    ...
    // Record status history for the rehire transition
    await tx`
      INSERT INTO app.employee_status_history (
        tenant_id, employee_id, from_status, to_status,
        effective_date, reason, created_by
      ...
      VALUES (
        ${context.tenantId}::uuid, ${employeeId}::uuid,
        'terminated'::app.employee_status, 'pending'::app.employee_status,
        ${rehireDate}::date, 'Rehire', ${updatedBy}::uuid
      )
    `;
