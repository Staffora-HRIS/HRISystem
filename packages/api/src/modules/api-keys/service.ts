...
/**
 * Hash an opaque API token with SHA-256 for storage.
 *
 * SECURITY NOTE: SHA-256 is the correct algorithm here because the input is a
 * high-entropy (256-bit) cryptographically random opaque token generated by the
 * server, NOT a user-chosen password. Slow KDFs (bcrypt, scrypt, argon2) exist
 * to defend against brute-force attacks on low-entropy human-chosen secrets;
 * they provide no additional security for full-entropy random tokens and would
 * impose unacceptable latency on every authenticated request. This matches the
 * industry-standard approach used by GitHub PATs, Stripe API keys, AWS access
 * keys, and similar opaque-token systems.
 *
...
/**
 * Extract the display prefix from a token (first 8 characters).
 */
...
export class ApiKeyService {
  ...
  , data: CreateApiKey
    ...
    // Generate token, hash, and prefix
    const fullToken = generateApiToken();
    const tokenHash = hashApiToken(fullToken);
    const keyPrefix = extractTokenPrefix(fullToken);
  ...
  async rotateApiKey(ctx: TenantContext, id: string): Promise<ServiceResult<ApiKeyCreatedResponse>> {
    ...
    const newFullToken = generateApiToken();
    const newTokenHash = hashApiToken(newFullToken);
    const newTokenPrefix = extractTokenPrefix(newFullToken);
    const newRow = await this.db.withTransaction(ctx, async (tx) => {
      await this.repository.revoke(ctx, id, tx);
      const created = await this.repository.create(ctx, {
        name: existing.name, keyHash: newTokenHash, keyPrefix: newTokenPrefix,
    ...
    return {
      success: true,
      data: {
        id: newRow.id, tenant_id: newRow.tenantId, name: newRow.name, key: newFullToken, key_prefix: newTokenPrefix,
  ...
  async validateApiKey(
    ...
    const row = await this.repository.findByKeyHash(tokenHash);
