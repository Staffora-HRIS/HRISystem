...
CREATE INDEX IF NOT EXISTS idx_employees_hire_date_status
  ON app.employees (tenant_id, hire_date)
  WHERE status != 'terminated';

CREATE INDEX IF NOT EXISTS idx_employees_termination_date_status
...
CREATE INDEX IF NOT EXISTS idx_employees_termination_reason
  ON app.employees (tenant_id, termination_reason)
  WHERE status = 'terminated';

-- Optimize retirement projection (employee_personal date_of_birth lookups)
CREATE INDEX IF NOT EXISTS idx_employee_personal_dob_active
...
CREATE INDEX IF NOT EXISTS idx_positions_org_unit_headcount
  ON app.positions (tenant_id, org_unit_id, headcount)
  WHERE is_active = true;

-- Optimize filled position counts (active primary assignments)
CREATE INDEX IF NOT EXISTS idx_position_assignments_primary_active
...
CREATE INDEX IF NOT EXISTS idx_requisitions_open_org_unit
